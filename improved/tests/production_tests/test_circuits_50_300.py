#!/usr/bin/env python3
"""
Script to reproduce and test circuits 50 and 300 from step2 debug files.

This script reconstructs the circuit knitting for step2, extracts only circuits
50 and 300, runs them with their original seeds, and outputs results to a new
debug file for comparison with the original debug files.

Expected behavior: Results should match test1 and test2 (which are identical for these circuits)
"""
...
target_circuits = {
    50: {'simulator_seed': 92, 'transpiler_seed': 92, 'prefactor': 0.0625},
    300: {'simulator_seed': 342, 'transpiler_seed': 342, 'prefactor': 0.0625},
}
...
debug_filename = "debug_circuits_50_300_reproduction.txt"

# Write header
with open(debug_filename, 'w') as debug_fh:
    debug_fh.write(f"# Circuit Knitter Debug Output\n")
    debug_fh.write(f"# Reproduction of circuits 50 and 300 from step2 debug files\n")
