...
def save_step1_fermion_number():
    ...
    sim_seed = 42
    bs_seed = 42
    counts_filename = os.path.join(results_dir, f"step1_counts_seed{sim_seed}.json")
    ...
    summary_filename = os.path.join(results_dir, f"step1_summary_seed{sim_seed}.json")
    with open(summary_filename, 'w') as f:
        json.dump(summary, f, indent=2)
    # print(f"Saved summary to {summary_filename}")
    
    # Save counts as CSV for easy plotting
    csv_filename = os.path.join(results_dir, f"step1_counts_seed{sim_seed}.csv")
...
def save_step1_fermion_number_knitted():
    ...
    counts_filename = os.path.join(results_dir, f"step1_knitted_counts_sim{sim_seed}_tp{tp_seed}_bs{bs_seed}.json")
    with open(counts_filename, 'w') as f:
        json.dump(counts, f, indent=2)
    ...
    # Save counts as CSV for easy plotting
    csv_filename = os.path.join(results_dir, f"step1_knitted_counts_sim{sim_seed}_tp{tp_seed}_bs{bs_seed}.csv")
    with open(csv_filename, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['bitstring', 'count'])
        for bitstring, count in counts.items():
...
if __name__ == "__main__":
    ...
    fermion_num, error, counts_file, summary_file, csv_file, shots_file = save_step1_fermion_number()
    # print(f"\nFirst Trotter step fermion number: {fermion_num:.4f} +/- {error:.4f}")
    
    # print("\nRunning knitted first Trotter step fermion number analysis...")
    knitted_fn, knitted_err, k_counts_file, k_summary_file, k_csv_file, k_shots_file = save_step1_fermion_number_knitted()
