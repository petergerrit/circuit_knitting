...
def save_second_trotter_step_fermion_number():
    ...
    sim_seed = 42
    bs_seed = 42
    counts_filename = os.path.join(results_dir, f"second_trotter_counts_seed{sim_seed}.json")
    ...
    summary_filename = os.path.join(results_dir, f"second_trotter_summary_seed{sim_seed}.json")
    with open(summary_filename, 'w') as f:
        json.dump(summary, f, indent=2)
    print(f"Saved summary to {summary_filename}")
    
    # Save counts as CSV for easy plotting
    csv_filename = os.path.join(results_dir, f"second_trotter_counts_seed{sim_seed}.csv")
    ...
    shots_filename = os.path.join(results_dir, f"second_trotter_shots_seed{sim_seed}.txt")
...
def save_second_trotter_step_fermion_number_knitted():
    """Run the step 2 circuit through circuit knitter, measure fermion number, compute bootstrap error, and save."""
    ...
    counts_filename = os.path.join(results_dir, f"second_trotter_knitted_counts_sim{sim_seed}_tp{tp_seed}_bs{bs_seed}.json")
    with open(counts_filename, 'w') as f:
        json.dump(counts, f, indent=2)
    ...
    summary_filename = os.path.join(results_dir, f"second_trotter_knitted_summary_sim{sim_seed}_tp{tp_seed}_bs{bs_seed}.json")
    with open(summary_filename, 'w') as f:
        json.dump(summary, f, indent=2)
    print(f"Saved knitted summary to {summary_filename}")
    
    # Save counts as CSV for easy plotting
    csv_filename = os.path.join(results_dir, f"second_trotter_knitted_counts_sim{sim_seed}_tp{tp_seed}_bs{bs_seed}.csv")
    with open(csv_filename, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['bitstring', 'count'])
        for bitstring, count in counts.items():
    ...
    # Save each shot on its own line (num_shots total lines)
    shots_filename = os.path.join(results_dir, f"second_trotter_knitted_shots_sim{sim_seed}_tp{tp_seed}_bs{bs_seed}.txt")
    with open(shots_filename, 'w') as f:
        for bitstring, count in counts.items():
            for _ in range(int(count)):
                f.write(bitstring + '\n')
    print(f"Saved {num_shots} knitted shots to {shots_filename}")
...
if __name__ == "__main__":
    ...
    fermion_num, error, counts_file, summary_file, csv_file, shots_file = save_second_trotter_step_fermion_number()
    print(f"\nSecond Trotter step fermion number: {fermion_num:.4f} +/- {error:.4f}")
    
    print("\nRunning knitted second Trotter step fermion number analysis...")
    knitted_fn, knitted_err, k_counts_file, k_summary_file, k_csv_file, k_shots_file = save_second_trotter_step_fermion_number_knitted()
